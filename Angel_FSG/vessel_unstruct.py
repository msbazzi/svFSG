...
class Vessel():
    ...
    def __init__(self, **kwargs):
        ...
        self.mesh= kwargs.get("mesh","mesh/wall/mesh-complete.mesh.vtu")
        self.vtp_full= kwargs.get("vtp_full","mesh/wall/mesh-complete.mesh.vtp")
        self.surfaces_folder= kwargs.get("surfaces_folder","mesh/wall/mesh-surfaces")
        self.surfaces= kwargs.get("surfaces",["mesh/wall/mesh-surfaces/wall_inner.vtp",
                                               "mesh/wall/mesh-surfaces/wall_outer.vtp",
                                                "mesh/wall/mesh-surfaces/wall_inlet.vtp",
                                                "mesh/wall/mesh-surfaces/wall_outlet.vtp" ])
        self.radial_heat_flux =  kwargs.get("radial_heat_flux","simulationResults/heat_radial.vtu")
        self.axial_heat_flux =  kwargs.get("axial_heat_flux","simulationResults/heat_axial.vtu")
        self.configuration_file =  kwargs.get("configuration_file","FolderVesselConfigurationFiles/Native_in_handshake_")
        self.vesselReference = None
        self.vesselSolid = None #kwargs.get("vesselSolid","pipe/wall-mesh-complete/mesh-complete.mesh.vtu")
        self.vesselFluid = pv.read("mesh/lumen/mesh-complete.mesh.vtu")
    ...
    def runPrestressIteration(self):
        self.extractTraction()
        self.updateSolid()
        self.saveSolid()
        self.runPrestress()
    ...
    def runSolid(self):
        ...
        if self.timeIter == 0 and self.timeStep == 0:
            os.system("mpirun -n 10 ~/repo/svFSI/build/svFSI-build/bin/svFSI FolderSimulationInputFiles/solid_mm_unstruct.mfs")
             ...
             os.system("mpirun -n 10 " + self.simulationExecutable + " " + self.simulationInputDirectory + "/solid_aniso.mfs")
    
        print("Solid simulation finished.")
        os.system('cp 10-procs/result_'+str(self.resultNum)+'.vtu simulationResults/solid_' + str(self.timeStep) + '.vtu')

        return
    
    def runPrestress(self):
        os.system('rm -rf 10-procs/*')
        os.system("mpirun -n 10 ~/repo/svFSIplus/build/svFSI-build/bin/svFSI FolderSimulationInputFiles/solid_mm_prestress.xml")
        print("Prestress simulation finished.")
        os.system('cp 10-procs/result_'+str(self.resultNum)+'.vtu simulationResults/prestress.vtu')

        return

    def runFluid(self):
        os.system('rm -rf 10-procs/*')
        os.system("mpirun -n 10 " + self.simulationExecutable + " " + self.simulationInputDirectory + "/input_fluid.xml")

        with open('10-procs/histor.dat') as f:
            if 'NaN' in f.read():
                raise RuntimeError("Simulation has NaN!")
        print("Fluid simulation finished.")
        os.system('cp 10-procs/result_'+str(self.resultNum)+'.vtu simulationResults/fluid_' + str(self.timeStep) + '.vtu')
        return

    def runHeatT(self):
        os.system('rm -rf 10-procs/*')
        os.system("mpirun -n 10 " + self.simulationExecutable + " " + self.simulationInputDirectory + "/svFSI_heat_axial.xml")
        with open('10-procs/histor.dat') as f:
            if 'NaN' in f.read():
                raise RuntimeError("Simulation has NaN!")
        print("Heat transfer axial simulation finished.")
        os.system('cp 10-procs/result_050.vtu simulationResults/heat_axial.vtu')
        
        
        os.system('rm -rf 10-procs/*')
        os.system("mpirun -n 10 " + self.simulationExecutable + " " + self.simulationInputDirectory + "/svFSI_heat_radial.xml")
        with open('10-procs/histor.dat') as f:
            if 'NaN' in f.read():
                raise RuntimeError("Simulation has NaN!")
        print("Heat transfer radial simulation finished.")
        os.system('cp 10-procs/result_050.vtu simulationResults/heat_radial.vtu')
        return
    ...
    def compute_cell_centers(self,heat_flux_data):
        ...
        cellkey = list(heat_flux_data.cells_dict.keys())[0]
        cells = heat_flux_data.cells_dict[cellkey]  
    ...
    def extractTraction(self):
       
        print("Extracting traction...")

        fluidresult = "simulationResults/fluid_0.vtu"
        mesh = pv.read(fluidresult)
        
        surface = mesh.extract_surface()

        traction_data = surface.point_data["Traction"]

        surface.save('simulationResults/traction.vtp')

        print("Traction extracted and saved as 'traction.vtu' in the 'simulationResults' directory")
